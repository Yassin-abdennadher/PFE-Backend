...
router.post('/', authenticate, async (req: Request, res: Response) => {
    ...
    if (!piece) return res.status(400).json({ message: 'Erreur création piece' });
...
router.get('/', authenticate, async (req: Request, res: Response) => {
    ...
    if (!pieces) return res.status(400).json({ message: 'Aucune piece Trouvé' });
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
    ...
    if (!piece) return res.status(400).json({ message: 'Aucune piece Trouvé' });
