...
export const createPiece = async (data: any) => {
    try {
...
export const getAllPiece = async () => {
    try {
        ...
        if (!pieces || pieces.length === 0) throw new Error('Aucune piece Trouvé');
        return pieces;
    } catch (err: any) {
        return err.message;
    }
...
export const getPieceById = async (id: string) => {
    try {
        ...
        if (!piece) throw new Error('piece non Trouvé');
        return piece;
    } catch (err: any) {
        return err.message;
    }
...
export const updatePiece = async (id: string, updateData: any) => {
    try {
        if (!id) throw new Error('aucun id donnée');
        ...
        if (!piece) throw new Error(' Aucune piece Trouvé ');
        return piece;
    } catch (err: any) {
        return err.message
    }
