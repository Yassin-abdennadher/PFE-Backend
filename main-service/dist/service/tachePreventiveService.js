...
const calculerNouvelleDate = (date, frequence) => {
...
export const createTachePrev = async (data) => {
    try {
        if (!data)
...
export const getAllTachePrev = async () => {
    try {
        ...
        if (!tachesPrev || tachesPrev.length === 0)
...
export const updateTachePrev = async (id, updateData) => {
    try {
        if (!id)
            throw new Error('aucun id donnée');
        const tachePrev = await TachePreventive.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true });
        if (!tachePrev)
            throw new Error(' Aucune Tache Trouvé ');
        return tachePrev;
    }
...
export const deleteTachePrev = async (id) => {
    try {
        if (!id)
            throw new Error('aucun id donnée');
...
//# sourceMappingURL=tachePreventiveService.js.map