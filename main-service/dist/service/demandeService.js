...
export const createDemande = async (data) => {
    try {
        if (!data)
            throw new Error("Aucune Demande a Ajouter");
        const demandeToAdd = {
            userId: data.userId,
            titre: data.titre,
...
export const getAllDemande = async () => {
    try {
        ...
        if (!demande || demande.length === 0)
            throw new Error("Aucune Demande Trouvé");
        return demande;
    }
    catch (err) {
        return err.message;
...
export const getDemandeById = async (id) => {
    try {
        ...
        if (!demande)
            throw new Error('demande Non Trouvé');
        return demande;
    }
    catch (err) {
        return err.message;
...
export const updateDemande = async (id, updateData) => {
    try {
        const demande = await Demande.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true });
        if (!demande) {
...
//# sourceMappingURL=demandeService.js.map