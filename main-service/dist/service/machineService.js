...
export const createMachine = async (data) => {
    try {
        if (!data)
            throw new Error("Aucune Machine a Ajouter");
        const machineToAdd = {
            nom: data.nom,
            type: data.type,
            marque: data.marque,
...
export const getAllMachine = async () => {
    try {
        ...
        if (!machine || machine.length === 0)
            throw new Error("Aucune Machine Trouvé");
        return machine;
    }
    catch (err) {
        return err.message;
...
export const getMachineById = async (id) => {
    try {
        ...
        if (!machine)
            throw new Error('Machine Non Trouvé');
        return machine;
    }
    catch (err) {
        return err.message;
...
export const updateMachine = async (id, updateData) => {
    try {
        const machine = await Machine.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true });
        if (!machine) {
